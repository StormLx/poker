    ...
    useEffect(() => {
        ...
        const setupEventListeners = (socket) => {
            ...
            onVotingReset(({participants, votesRevealed, statistics}) => {
                ...
                setRoomState(prev => prev
                    ? {...prev, participants, votesRevealed, statistics, creatorLiveVotes: {}}
                    : null);
            });
            ...
            onParticipantUpdated(({participant: updatedParticipant}) => {
                ...
                if (updatedParticipant.id !== (getSocketInstance() ? getSocketInstance().id : null)) {
                    addToast(`User ${updatedParticipant.name} ${updatedParticipant.isSpectator
                        ? 'is now a spectator.'
                        : 'is no longer a spectator.'}`, 'info');
                }
            });
        };
                ...
                if (roomFromUrl && !currentUserName) {
                    ...
                    socketJoinRoom(roomIdToRejoin, currentUserName, (response) => {
                        if (response.success) {
                            ...
                            handleRoomEntry(response.room, currentUserName, response.room.creatorId === (currentSocket
                                ? currentSocket.id
                                : null));
                        } else {
        ...
        return () => {
            if (statusUnsubscribe) {
                statusUnsubscribe.unsubscribe
                    ? statusUnsubscribe.unsubscribe()
                    : connectionStatusEmitter.unsubscribe(statusUnsubscribe);
            }
            ...
            <main>
                {!showAppContent && currentView !== 'enterNameForRoomJoin' && ( // Keep enterNameForRoomJoin visible during connection issues if already there
                    <div className="connection-error-fullscreen">
                        ...
                        <p>Unable to connect to the server. Please check your internet connection or try again
                            later. </p>
